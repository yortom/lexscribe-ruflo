...
**Lexscribe** es una plataforma de automatización de contratos y documentos legales tipo (despacho de abogados, mono-usuario MVP, salida `.docx`, idioma español). Plantillas con variables `{{objeto.campo}}`, expedientes que agrupan documentos/contactos/fechas/facturación, cláusulas tipo reutilizables, calendario y esquema dinámico de variables.

### Documentos fuente de verdad (obligatorios)

Antes de proponer, diseñar o implementar cualquier funcionalidad, **lee y referencia** el documento correspondiente. Si entra en conflicto con esta CLAUDE.md, **manda el documento de proyecto**. Toda decisión funcional, de datos o de arquitectura debe registrarse en el documento que toque, no en código suelto ni en memoria del agente.

| Documento | Qué contiene | Cuándo consultarlo |
|-----------|--------------|---------------------|
| [`docs/FUNCIONAL.md`](docs/FUNCIONAL.md) | **Qué hace** la plataforma: visión, módulos (4.1–4.7), catálogo de features `F-XXX` con estado/prioridad, sintaxis de variables (§5.2), flujos `FL-1`–`FL-13`, fuera de alcance, preguntas abiertas. | Cualquier cambio o duda sobre **comportamiento de usuario**, alcance MVP, qué features existen, o cómo encajan los flujos. **Toda nueva funcionalidad se registra aquí antes de implementarse.** |
| [`docs/DATOS.md`](docs/DATOS.md) | **Cómo se persisten** los conceptos: stack (MongoDB + MinIO), convenciones (soft-delete, snapshots, auditoría), inventario de colecciones (`expedientes`, `contactos`, `plantillas`, `clausulas`, `documentos`, `eventos`, `facturas`, `esquemas`, `usuarios`, `auditoria`). Cada campo trazado a una `F-XXX`. | Cualquier cambio de modelo, nueva colección/atributo, índices, integridad referencial, o lectura del esquema. |
| [`docs/ARQUITECTURA.md`](docs/ARQUITECTURA.md) | **Stack y estructura del sistema**: Next.js (frontend) + NestJS (backend) + MongoDB + MinIO + Nginx, todo en NAS del despacho, backup a Google Drive vía rclone. Auth JWT, generación `.docx` con `docxtemplater`, editor con CodeMirror 6, módulos NestJS por bounded context. | Decisiones de stack, estructura de módulos, despliegue, autenticación, generación de documentos, infra. |

### Reglas de uso de los documentos

1. **Lee primero, propón después.** Antes de tocar código que afecte a una feature, abre el documento correspondiente y localiza la `F-XXX` o sección.
2. **Trazabilidad por `F-XXX`.** Cuando implementes algo, referencia la feature (`F-012`, `FL-6`, etc.) en commits y PRs.
3. **Si falta info, no la inventes.** Si algo no está en los documentos, regístralo como **pregunta abierta** en `FUNCIONAL.md §8` o consulta al usuario antes de decidir.
4. **Cambios funcionales → changelog.** Toda modificación de los 3 documentos añade entrada en su sección de changelog con fecha (formato `YYYY-MM-DD`).
5. **Conflictos:** los documentos `docs/*.md` mandan sobre suposiciones de código. Orden de precedencia: `FUNCIONAL > DATOS > ARQUITECTURA` (qué antes que cómo, datos antes que infra).
